...
case class UncompressedBlock(length: Int, data: IndexedSeq[Char]) extends Block
case class CompressedBlock(length: Int, data: Char) extends Block

class RleEncoding {

  def encode(str: String): IndexedSeq[Block] = {
    val (prev, optBlock, result) =
      str.toCharArray.foldLeft((None: Option[Char], None: Option[Block], IndexedSeq.empty[Block])) {
        ...
        case ((Some(prev), None, result), char) =>
            (Some(char), Some(UncompressedBlock(1, IndexedSeq(prev))), result)
    ...
    (optBlock, prev) match {
      ...
      case (None, Some(prev)) => result :+ UncompressedBlock(1, IndexedSeq(prev))
      case (None, None) => IndexedSeq.empty[Block]
    }
